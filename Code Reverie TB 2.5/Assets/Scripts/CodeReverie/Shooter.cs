...
namespace CodeReverie
{
    public class Shooter : SerializedMonoBehaviour
    {
        ...
        [SerializeField] public float startingDistance = 0.1f;
        [SerializeField] public float timeBetweenBursts;
        [SerializeField] public float restTime = 1f;
        [SerializeField] public bool stagger;

        [Tooltip("Stagger must be enabled for oscillate to function properly.")] [SerializeField]
        ...
        private void TargetConeOfInfluence(out float startAngle, out float currentAngle, out float angleStep,
            out float endAngle)
        {
            Vector2 targetDirection = PlayerManager.Instance.currentParty[0].characterController.transform.position - transform.position;
