...
namespace CodeReverie
{
    public class UILineRenderer : Graphic
    {
        ...
        protected override void OnPopulateMesh(VertexHelper vh)
        {
            ...
            // points.Insert(0, this.gameObject);
            ...
            for (int i = 0; i < points.Count - 1; i++)
            {
                ...
                //     CreateLineSegment(gameObject, points[i], vh);
                // }
                // else
                // {
                //     // Create a line segment between the next two points
        ...
        /// <param name="point2">The endint point of the segment</param>
        /// <param name="vh">The vertex helper that the segment is added to</param>
        private void CreateLineSegment(GameObject point1, GameObject point2, VertexHelper vh)
        {
            ...
            // Create the start of the segment
            Quaternion point1Rotation = Quaternion.Euler(0, 0, RotatePointTowards(point1.transform.localPosition, point2.transform.localPosition) + 90);
            vertex.position = point1Rotation * new Vector3(-thickness / 2, 0);
            vertex.position += point1.transform.localPosition - offset;
            vh.AddVert(vertex);
            vertex.position = point1Rotation * new Vector3(thickness / 2, 0);
            vertex.position += point1.transform.localPosition - offset;
            vh.AddVert(vertex);
            ...
            // Create the end of the segment
            Quaternion point2Rotation = Quaternion.Euler(0, 0, RotatePointTowards(point2.transform.localPosition, point1.transform.localPosition) - 90);
            vertex.position = point2Rotation * new Vector3(-thickness / 2, 0);
            vertex.position += point2.transform.localPosition - offset;
            vh.AddVert(vertex);
            vertex.position = point2Rotation * new Vector3(thickness / 2, 0);
            vertex.position += point2.transform.localPosition - offset;
            vh.AddVert(vertex);
